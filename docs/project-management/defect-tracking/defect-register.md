...
| PDEF-006 | [#292](https://github.com/zhoujuxi2028/michael-zhou-qa-portfolio/issues/292) | `pre-push` hook 缺失 workflow-doc-sync 检查：开发者本地 push 新 workflow 文件时，`check-workflow-doc-sync.sh` 未被调用，导致新增 workflow 未登记文档也能通过本地门控，只在 CI `repo-meta-ci / lint` 才暴露 | 仓库级 / CI + 流程 | P2 | 2026-05-26 | 在 `pre-push` hook 末尾追加 workflow-doc-sync 调用（`git diff GUARD_BASE..GUARD_HEAD` 中的 `.github/workflows/*.yml` 变更时触发）；提交于 `dd7f164c`（PR #292） | PR #292 commit `dd7f164c` |
| PDEF-007 | N/A | `claude-code-review.yml` / `copilot-setup-steps.yml` 在加入仓库时未在 README.md / CLAUDE.md 登记；PDEF-006 修复的 pre-push 检查仅覆盖**当前 push 新改动的** workflow 文件，无法追溯历史遗漏。Dependabot PR #298（`actions/checkout v4→v6`）触碰这两个文件，CI `repo-meta-ci / lint`（run [#26551798570](https://github.com/zhoujuxi2028/michael-zhou-qa-portfolio/actions/runs/26551798570) / job [#78215228958](https://github.com/zhoujuxi2028/michael-zhou-qa-portfolio/actions/runs/26551798570/job/78215228958)）报红；本地无报错原因：本地 push 未改动任何 workflow 文件，hook 检测列表为空直接跳过 | 仓库级 / docs 治理 | P2 | 2026-05-28 | 在 README.md 补录 `claude-code-review.yml`、`copilot-setup-steps.yml`、`claude.yml`；CLAUDE.md 补录 `copilot-setup-steps.yml`；已合并至 main via PR #333（commit `c69199d1`） | PR #333 commit `c69199d1` |
...
| 2026-05-28 | 登记并关闭 `PDEF-007`：`claude-code-review.yml` / `copilot-setup-steps.yml` 历史未登记文档，Dependabot PR #298 触发 CI lint 报红（run #26551798570）；pre-push hook 未报错原因：当次 push 无 workflow 文件变更，检测列表为空跳过；修复：README.md / CLAUDE.md 补录两文件，PR #333 已合并 | QA |
