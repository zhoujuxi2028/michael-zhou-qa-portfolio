...
| 修复项 | 文件 | 描述 |
|--------|------|------|
| Workaround 追踪规范 | `docs/process/workaround-tracking.md` | 完整规范：Issue 模板、生命周期、追踪表、检查命令 |
| CLAUDE.md 规则更新 | `CLAUDE.md` | 补充 deadline（今天+5天）和 P1 升级规则 |
| Label 策略更新 | `docs/process/label-strategy.md` | 收录 `workaround` label（颜色 #FF8C00） |

### 4.2 已完成的仓库侧操作

- [x] 已在 GitHub repo 创建 `workaround` label（颜色 `#FF8C00`，描述：临时绕过方案，必须附 deadline 并在期限内修复）

```bash
gh label list --search "workaround"
```
...
```bash
# 确认规范文档存在且内容完整
cat docs/process/workaround-tracking.md | grep -E "deadline|P1|workaround"

# 确认 CLAUDE.md 包含新规则
grep -A3 "临时 workaround 必须" CLAUDE.md

# 确认 label-strategy.md 包含 workaround label
grep "workaround" docs/process/label-strategy.md
```
...
| 风险 | 描述 | 缓解措施 |
|------|------|---------|
| 规则执行依赖人工检查 | 无自动化 PR 门控检查 workaround Issue | 通过 PR template / review checklist 持续强化，后续如有需要再补自动门禁 |
...
- [workaround-tracking.md](../../process/workaround-tracking.md) — Workaround 追踪规范（本次新建）
