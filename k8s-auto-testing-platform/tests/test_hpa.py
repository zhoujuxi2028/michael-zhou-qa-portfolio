...
class TestHPAScaling:
    ...
    def test_min_replicas_maintained(self, apps_v1_api, namespace, deployment_name, wait_helper):
        ...
        def check_min_replicas():
            ...
            logger.debug(f"Waiting for min replicas: {ready}/2")
            return ready >= 2

        met = wait_helper(check_min_replicas, timeout=120, interval=5)
        assert met, "Failed to reach minimum 2 replicas within timeout"
    ...
    def test_hpa_scale_up(
        ...
        if scaled:
            final_deployment = apps_v1_api.read_namespaced_deployment(name=deployment_name, namespace=namespace)
    ...
    def test_max_replicas_not_exceeded(self, autoscaling_v2_api, apps_v1_api, namespace, hpa_name, deployment_name):
        ...
        assert current_replicas <= max_replicas, f"Current replicas ({current_replicas}) exceeds max ({max_replicas})"
