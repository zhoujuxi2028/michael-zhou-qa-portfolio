...
class TestHPAScaling:
    ...
    def test_hpa_exists(self, autoscaling_v2_api, namespace, hpa_name):
        ...
        logger.info(
            f"HPA {hpa_name} exists with min={hpa.spec.min_replicas}, max={hpa.spec.max_replicas}"
        )
    ...
    def test_hpa_metrics_configured(self, autoscaling_v2_api, namespace, hpa_name):
        ...
        # Check CPU metric
        cpu_metric = next(
            (m for m in hpa.spec.metrics if m.resource.name == "cpu"), None
        )
        assert cpu_metric is not None
        assert cpu_metric.resource.target.average_utilization == 50

        logger.info("HPA metrics configured correctly")

    def test_min_replicas_maintained(
        self, apps_v1_api, namespace, deployment_name, wait_helper
    ):
    ...
    def test_hpa_scale_up(
        ...
        # Get service endpoint (verify service exists)
        _ = core_v1_api.read_namespaced_service(name=service_name, namespace=namespace)
        ...
        if scaled:
            ...
            logger.info(
                f"Scale up successful: {initial_replicas} -> {final_replicas} replicas"
            )
            assert final_replicas > initial_replicas
        else:
            logger.warning(
                "HPA did not scale up within timeout (this is expected without real load)"
            )
    ...
    def test_hpa_scale_down(self, apps_v1_api, namespace, deployment_name, wait_helper):
        ...
        if current_replicas > 2:
            ...
            if scaled:
                ...
                logger.info(
                    f"Scale down successful: {current_replicas} -> {final_replicas} replicas"
                )
    ...
    def test_max_replicas_not_exceeded(
        self, autoscaling_v2_api, apps_v1_api, namespace, hpa_name, deployment_name
    ):
    ...
    def test_hpa_status(self, autoscaling_v2_api, namespace, hpa_name):
        ...
        if hpa.status.current_metrics:
            for metric in hpa.status.current_metrics:
                if metric.resource:
                    logger.info(
                        f"  {metric.resource.name}: {metric.resource.current.average_utilization}%"
                    )
