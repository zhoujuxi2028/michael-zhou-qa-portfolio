...
```
  ...
  Client ──REST──→ ┌──────────────┐  REST (sync)  ┌──────────────┐ │
                   ...
                   │   :3005      │  PATCH /api/orders/:id/status  │
                   └──────────────┘                                 │
                          │                                         │
...
| 服务 | 端口 | 职责 | 数据库 |
|------|------|------|--------|
| Order Service | 3003 | 订单生命周期管理、编排入口 | SQLite (orders.db) |
| Inventory Service | 3004 | 库存查询/扣减/回滚 | SQLite (inventory.db) |
| Payment Service | 3005 | 支付处理、事件驱动 | SQLite (payments.db) |
| Redis | 6379 | 消息总线 | - |
