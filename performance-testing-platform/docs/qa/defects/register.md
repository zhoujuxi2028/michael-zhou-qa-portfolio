...
| DEF-022   | [PR #257](https://github.com/zhoujuxi2028/michael-zhou-qa-portfolio/pull/257) | `Conventional Commits (subject rules)` CI 红：3 个 commit subject 不合规       | P1 / High   | ✅ Blocking        | 2026-05-24 | 🟡 Open（需仓库维护者 force-push 或调整 PR base） | — | 违规 commits（来自 `feature/performance-testing` 祖先，被 PR #257 base=`main` 一并纳入 `BASE..HEAD --no-merges` 范围）：`b1dd0ea`（subject 含 em-dash `—`，不匹配格式正则；长度 95）、`ddbd35e`（长度 74 > 72）、`cd4d883`（长度 75 > 72）。Agent 已尝试 `git filter-branch` 重写 3 个 subject 通过本地 CI 复现校验，但 `report_progress` 推送时的自动 rebase 将重写过的 commit 视为 patch-id 重复并跳过，最终 force-push 失败（Agent 受策略限制无法直接 `git push --force`）。**两种解锁方式（任选其一）**：① 仓库维护者本地执行 `git checkout copilot/fix-defect-tracking-issues && git filter-branch -f --msg-filter '<rewrite-sed>' 9b8d5a8..HEAD && git push --force-with-lease`（具体 sed 脚本见本 PR 描述）；② 在 GitHub UI 将 PR #257 base 从 `main` 改为 `feature/performance-testing`（最简：BASE..HEAD 仅含 `de05c24` 一个 commit，CI 立即转绿，且语义更准确——本 PR 的 DEF-019 修复本应进入 `feature/performance-testing`）。建议方案 ②。 |
