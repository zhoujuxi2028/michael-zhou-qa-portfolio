...
| DEF-022   | [PR #255](https://github.com/zhoujuxi2028/michael-zhou-qa-portfolio/pull/255) | `Conventional Commits (subject rules)` CI 红：3 个 commit subject 不合规       | P1 / High   | ✅ Blocking        | 2026-05-24 | 🟡 Open（需 PR 维护者 force-push） | — | 违规 commits：`b1dd0ea`（subject 含 em-dash `—`，不匹配格式正则）、`ddbd35e`（subject 74 chars > 72）、`cd4d883`（subject 75 chars > 72）。修复方案：`git rebase -i ac55e60` 对 3 个 commit 执行 `reword`，改写为 ≤ 72 字符且无 em-dash 的 subject（如：`feat(hooks,ci): add commit-msg/secret-scan/multi-project/CI guard`、`fix(husky): P0 hardening for pre-commit/pre-push hooks`、`chore(perf): add pre-push hook for P-1 quality gates`），然后 `git push --force-with-lease`。该步需 PR #255 维护者本地执行 |
