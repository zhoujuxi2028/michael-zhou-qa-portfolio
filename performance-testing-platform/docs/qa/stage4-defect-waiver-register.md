...
| DEF-011   | [#230](https://github.com/zhoujuxi2028/michael-zhou-qa-portfolio/issues/230) | `baseline-export.js` 不兼容当前 k6 summary `p(95)` 字段                | P1 / High | ✅ Blocking                  | 2026-04-27 | 🟢 Fixed           | —           | 修复：将解析逻辑抽到 `src/utils/baseline.js` 的 `extractBaselineFromSummary` 纯函数，兼容 `values['p(95)']` / 顶层 `'p(95)'` / `'p(0.95)'`、`http_req_failed.value` 与 `http_reqs.count\|value`；新增 `validateBaseline` schema 校验；测试分层（9 条纯函数单测 + 4 条 schema 单测 + 6 条 CLI 集成测试）。Commit: _(本次)_ |
