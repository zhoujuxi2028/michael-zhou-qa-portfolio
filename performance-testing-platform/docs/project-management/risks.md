...
| R-26     | CI/CD    | CI coverage threshold 误判：代码重构导致覆盖率下降但功能正常                       | 中     | 中     | 🟡     | CI-COV-01       | 设置合理阈值 (80%)；提供覆盖率 diff 分析；允许小幅波动                                                                    |
| R-27     | CI/CD    | GitHub Actions artifact 丢失或损坏导致基线对比失败                               | 高     | 低     | 🟢     | CI-BASE-02      | 实现本地 baseline 模拟测试；artifact 下载失败时使用本地备份                                                          |
| R-28     | CI/CD    | 趋势数据持续增长导致存储空间不足                                               | 中     | 中     | 🟡     | CI-TREND-02     | 实现 90 天自动清理策略；定期备份历史数据                                                                               |
| R-29     | CI/CD    | Nightly cron 触发时并发运行导致资源冲突                                       | 高     | 中     | 🟡     | CI-SCHED-08     | 使用 lock/mutex 机制确保单实例运行；设置超时保护                                                                       |
| R-30     | CI/CD    | PR comment integration 失败导致测试结果无法展示                                 | 中     | 低     | 🟢     | CI-PR-01        | 实现 fallback 机制（失败时在 PR body 中显示）；日志记录失败原因                                                       |
| R-31     | CI/CD    | Grafana JSON API 访问受限导致趋势面板无法展示                                 | 中     | 低     | 🟢     | CI-TREND-03     | 实现 fallback 方案；使用 InfluxDB 直接查询备选                                                                          |
| R-32     | CI/CD    | 定时测试邮件通知被误判为垃圾邮件                                             | 低     | 中     | 🟢     | CI-SCHED-04     | 使用 GitHub official actions 发送通知；添加邮件认证                                                                   |
| R-33     | CI/CD    | Coverage threshold 设置过高导致开发效率低下                                   | 低     | 中     | 🟢     | CI-COV-10       | 从 80% 开始，逐步调整；提供覆盖率 exempt 机制                                                                          |
| R-34     | CI/CD    | Trend analysis 数据量过大导致分析性能下降                                     | 低     | 低     | 🟢     | CI-TREND-01     | 实现数据采样策略；使用增量更新而非全量分析                                                                             |
| ~~R-25~~ | ~~工具~~ | ~~自检脚本质量缺陷（uptime 解析/grep 模式/code 一致性）~~                           | —      | —      | ✅     | 全局            | **已解决**: Stage 4 验证自检脚本本身，发现并修复 3 处 bug；commit 系列 + Issue #112 已关闭 (见 H-19)              |
...
| H-19 | 自检脚本 uptime 解析和 grep 模式 bug (Issue #112)            | Stage 4 stage4-selftest.sh 本身有 3 处 bug：line 65 uptime 浮点数解析，line 168 grep 大小写，line 174 uptime 方式不统一。修复：统一改为 awk 解析，grep 模式改为大小写精确匹配。commits 2d51a850, f4066577, ed56e555   | 2026-04-16 |
| H-20 | CI/CD pipeline reliability verification                 | Phase 7 实现前需要验证 CI 工作流程的可靠性，避免假绿灯和假红灯。通过本地模拟 CI 环境，测试 artifact 上传下载、baseline 对比、trend 收集等关键流程 | 2026-04-18 |
| H-21 | Phase 7 soak 委托缺失导致 Unit Tests 失败 (PR #171)     | `scripts/integration-test.sh` 未同步接入 `--phase soak` 专用委托，导致 `integration-test-phase7-soak.test.js` 失败并阻塞 CI。修复：在主入口增加 `bash scripts/integration-test-phase7-soak.sh` 分支；本地 lint + 270/270 Jest 通过；详见 `postmortems/RCA-2026-04-22-phase7-soak-delegation-regression.md` | 2026-04-22 |
