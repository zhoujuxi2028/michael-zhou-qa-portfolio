...
## 9. 影响升级与最终修复（2026-04-27 13:10 UTC 补充）

### 9.1 影响升级
PR #232 在补丁未落地的情况下被 squash 合并到 `main`（commit `bf0dca3`），导致：
- `main` 上 Run [24991866688](https://github.com/zhoujuxi2028/michael-zhou-qa-portfolio/actions/runs/24991866688) `Performance Testing / Shell Tests (Integration)` job 73179146443 红
- 所有派生 PR 继承红色，例如 PR #236 Run [24993769742](https://github.com/zhoujuxi2028/michael-zhou-qa-portfolio/actions/runs/24993769742) job 73185485705
- 失败模式从 fast bats 单 case red 升级为 integration bats `setup_file` 整体崩溃（`bash "$SCRIPT"` 因 `exit $FAIL`=1 退出 → bats 报 `Executed 1 instead of expected 7 tests`）

### 9.2 最终落地
鉴于 PR #232 已合并（新路径 `docs/qa/reports/execution/` 已在 main 存在），先前 §8 的 branch-discipline 限制不再适用；本次 agent session 在 `copilot/record-analyze-solve-issue` 分支直接落地 §5.1 修复（等价于 `attachments/DEF-011-fix-stage4-paths.patch`）：

| 文件 | 改动 |
|------|------|
| `scripts/stage4-selftest.sh` | `REPORT` 路径改为 `execution/`；新增 `mkdir -p "$(dirname "$REPORT")"`；§8.1 检查路径同步迁移 |
| `tests/unit/scripts/stage4-selftest-integration.bats` | `REPORT` 路径同步 |
| `tests/unit/scripts/stage4-selftest.test.sh` | `REPORT` 路径 + §8.1 / §报告生成 共 4 处同步 |
| `tests/unit/scripts/stage4-selftest-fast.bats` | 已在 PR #232 后续 commit 中独立修复（line 126 已是新路径），无需再改 |

### 9.3 本地回归证据
```
$ bash scripts/stage4-selftest.sh   # 沙箱 fetch-depth=1 受限，9.2 因浅克隆 FAIL（CI fetch-depth=50 不受影响）
exit=1 (沙箱独有), §8.1 ✅ PASS（核心修复点）

$ bats tests/unit/scripts/stage4-selftest-fast.bats
1..14
ok 1..13 ... not ok 14 9.2 (沙箱浅克隆，CI 已用 fetch-depth=50 规避)

$ grep -rn 'docs/qa/reports/(phase6-stage4-verification-report|stage4-selftest-report)\.md' \
       performance-testing-platform/scripts performance-testing-platform/tests
# 无匹配 ✅
```

### 9.4 CI 回归预期
PR #236 重跑 `Performance Testing CI` 后，`Performance Testing / Shell Tests (Integration)` 应转绿（setup_file 不再因 `exit 1` 而崩溃，全部 7 个 @test 正常执行）；`Shell Tests (Fast)` 维持绿。

### 9.5 关闭条件
- PR #236 合并后，`main` 上 `Shell Tests (Integration)` 转绿
- `defects/register.md` 把 DEF-011 从 Active 移至 Closed，写入关闭 commit
- AI-2 / AI-3 改进项继续按原 owner 推进
