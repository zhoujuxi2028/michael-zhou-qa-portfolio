# Implementation Plan — Phase 2: 系统指标采集 + 容量测试
...
│                    容量测试运行时                       │
│                                                       │
│  k6 capacity.k6.js ──→ Express API ←── /metrics      │
│  (阶梯递增 VUs)          │                (进程指标)   │
│       │                  │                    │        │
│       │            ┌─────┴──────┐             │        │
│       │            │  SQLite    │             │        │
...
| # | Task | 依赖 | 预估文件 |
|---|------|------|---------|
| T1 | 扩展 `/metrics` 端点 (SM-01~03) | 无 | `src/middleware/metrics.js` |
...
| T6 | 运行容量测试 + 分析瓶颈 | T1~T5 | 测试报告 |
| T7 | 更新 RTM + 文档 | T6 | `docs/test-cases/rtm-jmeter.md` |

**并行可能:** T1+T3 可并行开发（无依赖关系）。
...
| CAP-05 | 二分法找到最大并发 | 确定满足 SLA 的最大 VUs |
