...
构建一个专项性能测试平台，展示 k6 负载测试能力，区别于 microservice-testing-platform 中已有的简单 k6 脚本。

| 维度 | 说明 |
|------|------|
| **定位** | Portfolio 第 10 个项目，填补「性能测试」类别空白 |
| **核心价值** | 展示 load/stress/spike/smoke 4 种测试模式 + 可观测性 |
| **差异化** | microservice 项目的 k6 只有 3 个辅助脚本；本项目是专项平台，含阈值、场景、CI gate、Grafana 可视化 |
...
| US-05 | DevOps      | 我想在 CI 中自动运行 smoke test 作为性能门禁 | CI pipeline 中 k6 smoke 失败则阻断             |
...
UC-03: CI 性能门禁
  ...
  步骤: lint → unit test → k6 smoke test
...
| CI Pipeline | lint → unit test → k6 smoke gate | P0 |
...
| 包含 | 不包含 |
|------|--------|
| k6 脚本 (4 种模式) | JMeter 脚本 |
| Express 目标 API | 真实外部 API |
| SQLite 内存数据库 | 持久化数据库 (PostgreSQL) |
| Grafana + InfluxDB (Docker) | 云端部署 |
| GitHub Actions CI | 其他 CI 平台 |
...
| 2 | 完整用户故事，use cases | ✅ US-01~06, UC-01~03 |
| 3 | Scope 已确认（Phase 划分、功能边界） | ✅ Phase 1/2 + 边界定义 |
| 4 | 可行性评估（本机环境、依赖工具、技术风险） | ✅ 4 项风险已识别 |
| 5 | 依赖已识别（需安装的工具、需引入的库） | ✅ k6 + 7 npm 包 + 4 CI Actions |
| 6 | 需求描述已产出 | ✅ 本文档 |
