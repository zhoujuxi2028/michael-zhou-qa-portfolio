...
| SM-11 | SQLite 文件模式 + WAL | Cluster 多 Worker 共享 DB 文件，WAL 支持并发读 |
...
| 包含 | 不包含 |
|------|--------|
| Express Cluster 模式 (多核) | Prometheus 集成 |
| SQLite 文件模式 + WAL (真实磁盘 I/O) | Grafana 实时面板 |
| 扩展 `/metrics` 端点 (进程级指标) | 分布式采集 |
| 系统级采集器脚本 (CPU/mem/disk/net) | 云端环境测试 |
| 容量测试 (二分法逼近) | |
| 测试质量保障 (预热/隔离/重复性) | |
| CSV 报告归档 | |
