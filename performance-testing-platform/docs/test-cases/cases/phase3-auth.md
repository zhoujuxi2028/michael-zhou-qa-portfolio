...
| 用例 ID | 验证项 | 预期结果 | 验证方式 |
|---------|--------|---------|---------|
| AUTH-INT-01 | register → login → 获取 token | 注册成功 201，登录返回 accessToken + refreshToken | k6 1 VU (AUTH_ENABLED=true) |
| AUTH-INT-02 | 带 token 访问受保护 API | Bearer token 访问 POST /api/orders 返回 201 | k6 1 VU |
| AUTH-INT-03 | 无 token 访问受保护 API 被拒 | 返回 401 Unauthorized，不是 5xx | k6 1 VU |
