...
| 指标 | 值 |
|------|-----|
| **稳定 PASS** | ≤ 3500 VUs (R17a/R17b: p95=213/229ms; R13: p95=273ms) |
| **Inflection Point** | **~4000 VUs** — 1/2 PASS (485ms 边界), 1/2 FAIL (505ms) + R11 FAIL (686ms) |
| **稳定 FAIL** | 4400 VUs — 2/2 FAIL (p95: 635ms, 664ms) |
| **最大安全容量** | **~3500 VUs** |
| **主要瓶颈** | CPU-bound (peak 99.7-99.9%, event loop lag p95=197-438ms) |
| **内存** | 非瓶颈 (avg 58-65%, ~5-6.5GB 可用) |
| **磁盘** | 非瓶颈 (avg 4-6MB/s) |
| **网络** | 不适用 (本地 loopback 测试) |
| **写锁争用** | 清洁环境下未出现 — 旧结论已作废 |

> **建议:** 生产环境如需超过 3500 并发，需水平扩展至多节点 + 替换 SQLite 为 PostgreSQL/MySQL。
