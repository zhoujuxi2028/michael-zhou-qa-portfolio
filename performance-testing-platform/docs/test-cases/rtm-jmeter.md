...
| R21 | 5000 | 852.01ms | 0.00% | 4,357 | **❌ FAIL** | CPU avg 73.6% peak 100%, Mem avg 62.7%, event loop lag p95=610ms |
...
| 指标 | 值 |
|------|-----|
| **稳定 PASS** | ≤ 4000 VUs (R17/R18: p95=213~377ms, error 0%, 3000×2 + 4000×3) |
| **Inflection Point** | **4000~5000 VUs 之间** (4000 稳定 PASS，5000 p95=852ms FAIL) |
| **稳定 FAIL** | ≥ 5000 VUs — p95 超标，error rate 开始出现 |
| **最大安全容量** | **~4000 VUs** |
| **吞吐量天花板** | ~5,200-5,800 req/s (4000~6000 VUs 吞吐量平台，额外 VUs 只增加延迟) |
| **主要瓶颈** | Node.js Cluster event loop 处理上限 (lag p95: 200ms@3000 → 610ms@5000) |
| **内存** | 非瓶颈 (avg 62-63%) |
| **磁盘** | 非瓶颈 (avg 5-10MB/s) |
...
| 3000 | ~4,760/s | — | — |
| 4000 | ~5,260/s | +33% | +10% |
| 5000 | ~4,357/s | +25% | **-17%** ← 吞吐量反降，系统进入过载区 |
| 6000 | ~5,525/s | +20% | 波动 |

> **建议:** 生产环境如需超过 4000 并发，需水平扩展至多节点 + 替换 SQLite 为 PostgreSQL/MySQL。
