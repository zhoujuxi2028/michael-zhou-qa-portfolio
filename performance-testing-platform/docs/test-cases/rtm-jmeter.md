...
| 指标 | 值 |
|------|-----|
| **稳定 PASS** | ≤ 5000 VUs (R22/R23a/R23b: p95=345~352ms, error 0%, 一致性 1.2%) |
| **Inflection Point** | **5000~6000 VUs 之间** (5000 稳定 PASS×3，6000 稳定 FAIL×2) |
| **稳定 FAIL** | ≥ 6000 VUs — p95 超标 (660-736ms)，偶发错误 |
| **最大安全容量** | **~5000 VUs** |
| **吞吐量天花板** | ~6,500-6,750 req/s (5000~7000 VUs 吞吐量趋于平台) |
| **主要瓶颈** | Node.js Cluster event loop 处理上限 (lag p95: 200ms@3000 → 462ms@7000) |
| **内存** | 非瓶颈 (avg 61-63%) |
| **磁盘** | 非瓶颈 (avg 5-10MB/s) |
...
| 6000 | ~5,525/s | +20% | **-15%** ← 过载，吞吐反降 |
| 7000 | ~6,747/s | +17% | 波动，延迟已超标 |
