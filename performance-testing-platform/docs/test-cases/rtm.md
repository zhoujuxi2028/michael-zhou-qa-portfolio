...
## Phase 3 需求 → 设计 → 测试用例 (#56)

### JWT 认证场景

| 需求 ID | 需求 | 设计文件 | 测试用例 ID | 状态 |
|---------|------|---------|------------|------|
| AUTH-01 | 用户注册接口 | `src/routes/auth.js` | UT-AUTH-01~03 | PASS |
| AUTH-02 | 用户登录接口 | `src/routes/auth.js` | UT-AUTH-04~06 | PASS |
| AUTH-03 | Token 刷新接口 | `src/routes/auth.js` | UT-AUTH-07~08 | PASS |
| AUTH-04 | 用户登出接口 | `src/routes/auth.js` | UT-AUTH-09~10 | PASS |
| AUTH-05 | JWT 验证中间件 | `src/middleware/authenticate.js` | UT-MW-01~05 | PASS |
| AUTH-06 | 现有接口认证保护 (AUTH_ENABLED) | `src/routes/orders.js` | UT-MW-06~07 | PASS |
| AUTH-07 | k6 高并发登录压测 | `tests/performance/auth-login.k6.js` | AUTH-PERF-01 | PASS (100 VUs, p95=106ms) |
| AUTH-08 | k6 Token 刷新压测 | `tests/performance/auth-refresh.k6.js` | AUTH-PERF-02 | PASS (200 VUs, smoke verified) |
| AUTH-09 | k6 完整认证用户旅程 | `tests/performance/auth-journey.k6.js` | AUTH-PERF-03 | PASS (500 VUs, p95=86ms) |
| AUTH-10 | JMeter 认证压测 | `tests/jmeter/auth-load.jmx` | — | PASS (50 threads, 0% error) |
| AUTH-11 | 性能对比报告 | — | — | 待 T6 完整压测后产出 |
